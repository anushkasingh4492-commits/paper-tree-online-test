...
export async function POST(req: Request) {
  try {
    ...
    const admin = await getAcademyAdmin(academyId);
    ...
    if (
      ...
      new Date(settings.subscription_end) <
        new Date()
    ) {
      return NextResponse.json(
        {
          success: false,
          error:
            "The academy subscription has expired.",
        },
        { status: 403 }
      );
    }
    ...
    if (
      Number(settings.student_count) >=
      Number(settings.student_limit)
    ) {
      return NextResponse.json(
        {
          success: false,
          error:
            "The academy has reached its student limit.",
        },
        { status: 409 }
      );
    }
    ...
    // Validate selected batch, if one was provided.
    if (batchId) {
        ...
        return NextResponse.json(
          {
            success: false,
            error:
              "Selected batch was not found in this academy.",
          },
          { status: 404 }
        );
      }
    ...
    // Create student WITHOUT class_name.
    await pool.query(
      `
    ...
    // Assign student to batch, if selected.
    if (batchId) {
      await pool.query(
        `
