...
export async function POST(req: Request) {
  try {
    ...
    if (
      !["Class 11", "Class 12", "Class 11 + 12"].includes(
        className
      )
    ) {
      return NextResponse.json(
        {
          success: false,
          error: "Choose Class 11, Class 12 or Class 11 + 12.",
        },
        { status: 400 }
      );
    }
    ...
    if (settings.status !== "ACTIVE") {
      return NextResponse.json(
        {
          success: false,
          error: "This academy is not active.",
        },
        { status: 403 }
      );
    }
    ...
    if (
      ...
      new Date(settings.subscription_end) < new Date()
    ) {
      return NextResponse.json(
        {
          success: false,
    ...
    if (
      ...
      return NextResponse.json(
        {
          success: false,
          error: "The academy has reached its student limit.",
        },
        { status: 409 }
      );
    }
  ...
  try {
    ...
    if (!owned.rowCount) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: "Student not found in this academy.",
        },
        { status: 404 }
      );
    }
    ...
    await client.query(
      `DELETE FROM batch_students WHERE student_id = $1`,
      [studentId]
    );

    await client.query(
      `DELETE FROM notifications WHERE student_id = $1::text`,
      [studentId]
    );
